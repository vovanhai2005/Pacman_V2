...
void Texture::renderTileTexture(SDL_Renderer *&renderer, int tilePos, SDL_Rect *dstRect)
{
...
void Texture::loadCharacterTexture(SDL_Renderer *&renderer)
{
    ...
    else
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            PacmanUp[i % 3] = {posX, posY, 30, 30};
            posX += 31;
        }
        // Pacman Down
        for (int i = 0; i < 3; ++i)
        {
            PacmanDown[i % 3] = {posX, posY, 30, 30};
            posX += 31;
        }
        // Pacman Left
        for (int i = 0; i < 3; ++i)
        {
            PacmanLeft[i % 3] = {posX, posY, 30, 30};
            posX += 31;
        }
        // Pacman Right
        for (int i = 0; i < 3; ++i)
        {
            PacmanRight[i % 3] = {posX, posY, 30, 30};
            posX += 31;
        }
        // Pacman Dead
        posX = 0;
        ...
        {
            // Up Status
            ghost[i][UP][0] = {posX, posY, 30, 30}, posX += 31;
            ghost[i][UP][1] = {posX, posY, 30, 30}, posX += 31;
            // Down Status
            ghost[i][DOWN][0] = {posX, posY, 30, 30}, posX += 31;
            ghost[i][DOWN][1] = {posX, posY, 30, 30}, posX += 31;
            // Left Status
            ghost[i][LEFT][0] = {posX, posY, 30, 30}, posX += 31;
            ghost[i][LEFT][1] = {posX, posY, 30, 30}, posX += 31;
            // Right Status
            ghost[i][RIGHT][0] = {posX, posY, 30, 30}, posX += 31;
            ghost[i][RIGHT][1] = {posX, posY, 30, 30}, posX += 31;
            // Scared Status
            ghost[i][BLUE_SCARED][0] = {posX, 31, 30, 30}, posX += 31;
            ghost[i][BLUE_SCARED][1] = {posX, 31, 30, 30}, posX += 31;
            ghost[i][WHITE_SCARED][0] = {posX, 31, 30, 30}, posX += 31;
            ghost[i][WHITE_SCARED][1] = {posX, 31, 30, 30};
        ...
        // Dead Status
        posX = 248, posY = 62;
        ghost[GHOST_DEAD][UP][0] = {posX, posY, 30, 30};
        ghost[GHOST_DEAD][UP][0] = {posX, posY, 30, 30}, posX += 31;

        ghost[GHOST_DEAD][UP][0] = {posX, posY, 30, 30};
        ghost[GHOST_DEAD][UP][0] = {posX, posY, 30, 30}, posX = 248, posY += 31;
...
void Texture::renderPacmanTexture(SDL_Renderer *&renderer, int posX, int posY, int stat)
{
    ...
    if (stat != PACMAN_DEAD && pacmanFrame / 10 > 2)
        pacmanFrame = 0;
    switch (stat)
    {
    case -1:
