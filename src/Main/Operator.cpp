...
void Operator::render(SDL_Renderer *&renderer){
    ...
    else itemManage -> runEGBoard(renderer);
}
...
void Operator::inLoop()
{
    ...
    if (typeOfCoin != GameItemManage::notCoin) {
    ...
    if (!tickManage->isFrightenTime()) {
        ...
        if (greendy != nullptr) greendy->makeFrighten(false);
    }
    ...
    if (!pacman->isDead()) {
        ...
        else pinky->markDestination(Ghost::PINKY_CORNER_TILE_X, Ghost::PINKY_CORNER_TILE_Y);
